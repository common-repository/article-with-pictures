...
class Article_With_Pictures_Plugin
{
    ...
    public static function show_page()
    {
        ...
        <div class="wrap">
            ...
            <a style="font-size: 14px;margin-left: 10px;" href="https://www.ggdoc.cn/archives/66"
               target="_blank">
                使用教程
            </a>
                    ...
                    if (!empty($image_html) && !empty($article_with_pictures_options['content_image_type'])) {
                        ...
                        switch ($article_with_pictures_options['content_image_type']) {
                            ...
                            default:
                                ...
                                $content = preg_replace_callback('/(<\/p>|<\/div>|<br[^>]*>)/iU', function ($matches) use ($image_start_num, $image_html) {
                                    static $i = -1;
                                    $i++;
        ...
        if (!empty($attached_images)) {
            ...
            if (preg_match_all('/<img[^>]*src=[\'"]([^\'"]+)[\'"][^>]*>/iU', $content, $mat)) {
                foreach ($mat[1] as $img_src) {
                    ...
                    $attach_data = wp_generate_attachment_metadata($attach_id, ABSPATH . trim($img_src_info['path'], '/'));
    ...
    public static function generate_thumbnail($post_id, $post_title)
    {
        ...
        require_once(ABSPATH . 'wp-admin/includes/image.php');
        $attach_data = wp_generate_attachment_metadata($attach_id, $upload_dir['path'] . '/' . $img_filename);
        wp_update_attachment_metadata($attach_id, $attach_data);
        set_post_thumbnail($post_id, $attach_id);
        // 保存文章配图插件生成的数据
